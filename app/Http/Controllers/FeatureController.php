...
class FeatureController extends Controller
{
    ...
    {
        return view('back-end.pages.feature.feature');
    }

    public function FeatureEditData(Request $request)
    {
        $query = Feature::find($request->id); // Model will change
        if(!$query){
    ...
    public function FeatureInsert(Request $request)
    {
        ...
        if($request->has('delete')){
            $query = Feature::find($request->delete); // Model will change

            if ($query->image != null) {
                unlink(public_path('uploads/feature/' . $query->image)); // image directory name will change
            }

            $query->delete();

            $message = 'Feature Deleted Successfully!'; // message will change
        }else{
            ...
            $message = 'Feature Created Successfully!'; // Message will change

            if($request->has('id')){
                $query = Feature::find($request->id);  // Model name will change

                if ($request->hasFile('image')) {
                    if ($query->image != null) {
                        unlink(public_path('uploads/feature/' . $query->image)); // Directory name will change
                    }
                }

                $message = 'Feature Updated Successfully!'; // Message will change
                ...
                $query = new Feature;  // Model name will change
            ...
            if ($request->hasFile('image')) {
                ...
                $destinationPath    = public_path('uploads/feature/');  // Directory name will change
                ...
                // $upload_path       = "uploads/feature/";  // Directory name will change
    ...
    {
        $Feature = Feature::with('admin:id,name')->orderBy('id', 'desc')->get(); // Model & Variable will change
        $this->i=1;

        return DataTables::of($Feature)  // Variable will change
        ...
        ->addColumn('image', function ($data) {
                $url = asset('uploads/feature/'. $data->image);   // Directory name will change
