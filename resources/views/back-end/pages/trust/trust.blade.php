...
@section('pageTitle', isset($pageTitle) ? $pageTitle : 'All Trust List')
@section('content')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex">
                    <h5 class="box-title mr-auto p-2 mb-0">Trust</h5>
                    <button type="button" class="btn btn-primary btn-sm DataAddButton">Add Trust</button>
                </div>
                <div class="card-body">
...
<div class="modal fade" id="DataAdd" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <form id="DataAddUpdate" action="{{ route('admin.trust.insert') }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group mb-4">
                                        <label for="exampleInputTitle">Trust Title</label>
                                        <input class="form-control" type="text" name="title" placeholder="Trust Title">
                                        @error('title')
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group mb-4">
                                        <label for="exampleInputSlogan">Trust Slogan</label>
                                        <input class="form-control" type="text" name="slogan" placeholder="Trust Slogan">
                                        @error('slogan')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
...
<script>
    ...
    $('#dt-scroll-horizonal').DataTable({
        ...
        scrollX: true,
        ajax: {
            url: "{{ route('admin.trust.data') }}", // URL will change
            type: 'GET',
            cache: false,
            data: function(d) {
            }
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $(this).ajaxSubmit({
                    ...
                    , method: 'POST'
                    , dataType: 'json'
                    , url: "{{ route('admin.trust.insert') }}"  // URL will change
    ...
    $(document).on('click', '.tableEdit', function() {
        ...
        $(this).ajaxSubmit({
            ...
            , dataType: 'json'
            , method: 'GET'
            , url: "{{ route('admin.trust.edit') }}" // URL will change
            , success: function(responseText) {
